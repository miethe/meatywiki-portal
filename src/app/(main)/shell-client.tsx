 ...
 *
 * P2-04: Mounted <ArtifactPeekProvider> inside a Suspense boundary here so
 *        useArtifactPeek() + ?peek=<id> deep-links work across all (main) routes.
 *        Suspense is required because ArtifactPeekProvider calls useSearchParams()
 *        (Next.js App Router restriction). The fallback is null (silent — the
 *        provider is a side-effect shell that renders no visible UI on its own).
 */
...
import { ArtifactPeekProvider } from "@/components/artifact/ArtifactPeekProvider";
...
export function ShellClient({ children }: { children: React.ReactNode }) {
  ...
  return (
    ...
    <MobileNavContext.Provider value={{ isOpen: mobileNavOpen, toggle, close }}>
      {/*
       * ArtifactPeekProvider — P2-04 wiring.
       * Wraps the entire authenticated shell so any (main) page can call
       * useArtifactPeek() and the ?peek=<id> deep-link is handled app-wide.
       * Suspense is mandatory: useSearchParams() inside the provider opts the
       * subtree into dynamic rendering in Next.js App Router.
       * fallback={null} is intentional — the provider renders no visible chrome
       * of its own; children render immediately without a loading flash.
       */}
